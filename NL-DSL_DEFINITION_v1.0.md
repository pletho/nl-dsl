...
본 선언문에서는 NL-DSL의 철학적 기반과 자연어 기반 선언 흐름의 정체성을 강화하기 위하여,
...
* 한국어 명칭: NL-DSL (*자연어 기반 도메인 특화 선언문*)
* 영어 명칭: Natural Language - Domain Specific Language
* 일본어 예시: 自然言語ベースドメイン特化言語
* 스페인어 예시: Lenguaje Específico de Dominio basado en Lenguaje Natural

이는 단지 약어가 아니라, **사람이 스스로 이해하고 선언할 수 있는 프로그램 구조를 만들기 위한 언어 설계 철학의 기초 용어**이다.
...
NL-DSL의 alias는 문장 또는 역할 단위를 추상화한 "선언적 별칭"이다. 이를 통해 반복되는 블록을 외부 모듈화하거나, DSL의 재사용성과 구조적 명료성을 확보할 수 있다.
...
NL-DSL은 조건과 반복을 **문장 구조**와 **자연어 흐름**으로 표현한다.
...
NL-DSL의 파서는 문장을 해석하여 실행 가능한 구조로 변환한다.
...
NL-DSL은 선언형 구조로 병렬 실행을 명시할 수 있으며, 다음과 같은 패턴으로 병렬성을 표현할 수 있다.
